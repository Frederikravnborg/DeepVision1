...
selected_images = unique_images[:NUM_EXAMPLES]
...
for image_filename in selected_images:
    ...
    legend_elements = [
        Line2D([0], [0], color='green', lw=7, label='Ground Truth'),
        Line2D([0], [0], color='blue', lw=7, label='Positive Proposals'),
        Line2D([0], [0], color='red', lw=7, label='Negative Proposals')
    ]
    ax.legend(handles=legend_elements, loc='upper right', fontsize='larger')
    plt.savefig(f'{image_filename}', dpi=300)
    # plt.show()