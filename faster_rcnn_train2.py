    ...
    with torch.no_grad():
        for images, targets in tqdm(dataloader, desc="Evaluating", leave=False):
            ...
            for idx, output in enumerate(outputs):
                ...
                iou_matrix = box_iou(torch.tensor(gt_boxes, dtype=torch.float32), 
                                     torch.tensor(pred_boxes, dtype=torch.float32))

                # Check if any IoU values exceed the threshold
                iou_above_threshold = iou_matrix > iou_threshold
                if iou_above_threshold.numel() > 0:  # Check if it's not empty
                    gt_true_positive_counts = iou_above_threshold.sum().item()
                else:
                    gt_true_positive_counts = 0
                
                gt_total_positive_counts = iou_matrix.shape[0]  # Total ground-truth boxes

                total_true_positives += gt_true_positive_counts
                total_false_negatives += (gt_total_positive_counts - gt_true_positive_counts)
                total_false_positives += len(pred_boxes) - gt_true_positive_counts

    return total_true_positives, total_false_positives, total_false_negatives
...
for epoch in range(NUM_EPOCHS):
    ...
    true_positives, false_positives, false_negatives = evaluate_on_metrics(model, val_loader, device)
